    ...
    QMainWindow(parent)
{
   ...
   // highlighter = new MySyntaxHighlighter(MainWindow::textEdit->document());
    //highlighter = new MySyntaxHighlighter();
}
...
void MainWindow::openFile()
{
    ...
    if(!path.isEmpty()) {
        ...
        {
            Label *l=new Label(v[i].word,textEdit,rightW);//以当前单词新建一个label，并指定其父组件为rightW
            l->times = v[i].times;
            l->words = v[i].word;
            ...
            for(int j=0;j+v[i].times<R&&flag;j++)//遍历grid的每一行
                for(int ll=0;ll+v[i].times*v[i].word.size()<C;ll++)//遍历grid的每一列
                {
                    ...
                    {qDebug()<<v[i].times<<v[i].word<<j<<ll;
                        ...
                        g->addWidget(l,j,ll,v[i].times,v[i].times*v[i].word.size(),Qt::AlignAbsolute);//放置标签
                        //qDebug()<<label->width()<<label->height();,Qt::AlignTop|Qt::AlignRight,Qt::AlignAbsolute,Qt::AlignHCenter
                        flag=false;//已放置
                        break;//跳出内层循环
                    }
                }
            qDebug()<<l->width()<<l->height();
        }
